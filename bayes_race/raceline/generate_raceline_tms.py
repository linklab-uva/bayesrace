...
from bayes_race.tracks import TMS
...
N_BATCH = 40               # new observations after initialization
...
def optimize():
    ...
    if SAVE_RESULTS:
        ...
        np.savez(
            'results/{}_raceline_data-{}.npz'.format('TMS', savestr),
            y_ei=y_ei,
            y_nei=y_nei,
            y_rnd=y_rnd,
            iters=iters,
    ...
    if PLOT_RESULTS:
        ...
        plt.grid(True)
        plt.legend(loc=0)
        plt.savefig('results/{}_laptimes-{}.png'.format('TMS', savestr), dpi=600)
        plt.show()
